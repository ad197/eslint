...
const fs = require("fs"),
    ...
    ConfigArray = require("./simpleconfig/config-array"),
...
function processText(text, configHelper, filename, fix, allowInlineConfig, reportUnusedDisableDirectives, linter) {
    ...
    console.log("Will lint", filePath);
    console.dir(config);
    // return { result:{}, config };

    // Legacy: Manually load other plugins
    if (config.plugins && configHelpers.plugins) {
...
class CLIEngine {
    ...
    constructor(providedOptions) {
        ...
        // Look for simple config first
        const simpleConfigFilePath = findup("eslint.config.js", { cwd: this.options.cwd });
        if (simpleConfigFilePath) {
            debug(`Using simple config: ${ simpleConfigFilePath }`);
            
            this.config = new ConfigArray(require(simpleConfigFilePath), {
                basePath: path.dirname(simpleConfigFilePath)
            });

            console.log(JSON.stringify(this.config, null, "    "));
            return;
        } else {
            debug("Using .eslintrc.*");
